...
public static class Localization
{
    ...
    {
        ["LanSharingUnavailable"] = ("LAN sharing could not start. Check HTTPS/discovery ports and permissions. Local playback remains available; sharing will retry automatically.", "局域网共享未能启动，请检查 HTTPS/发现端口和权限。本机播放仍然可用，共享会自动重试。"),
        ["LanSharingReady"] = ("Encrypted sharing is running.", "加密共享正在运行。"),
        ["LanSharingDisabled"] = ("LAN sharing is off.", "局域网共享已关闭。"),
        ["LanStartService"] = ("Start or update the local OmniLyrics service to check sharing status.", "请启动或更新本机 OmniLyrics 服务，以检查共享状态。"),
        ["LanDevices"] = ("LAN devices", "局域网设备"),
        ["LanDevicesSubtitle"] = ("Pair devices securely and choose where your lyrics come from.", "安全配对设备，选择歌词来自哪台设备。"),
        ["LanSharing"] = ("Share this device", "共享此设备"),
        ["LanSharingHint"] = ("Off by default. Paired devices use encrypted HTTPS. Local CLI, TUI and GUI communicate without pairing. These settings apply immediately when saved.", "默认关闭。配对设备通过 HTTPS 加密通信。本机 CLI、TUI 和 GUI 无需配对。此页设置保存后立即生效。"),
        ["LanEnable"] = ("Allow paired devices to read local lyrics", "允许已配对设备读取本机歌词"),
        ["LanDeviceName"] = ("Device name", "设备名称"),
        ["LanSaveSharing"] = ("Save sharing settings", "保存共享设置"),
        ["LanInvite"] = ("Invite a device", "邀请设备"),
        ["LanInviteHint"] = ("Private invitation: single use, valid for 5 minutes. Send it directly to the device you trust. A new invitation replaces the previous one. Never publish it.", "私密邀请：五分钟有效，仅可使用一次。请直接传给信任的设备，不要公开分享。新邀请会替换旧邀请。"),
        ["LanAddress"] = ("This device’s LAN IP address", "本机局域网 IP 地址"),
        ["LanAllowControl"] = ("Also allow playback and favorites control", "同时允许控制播放和收藏"),
        ["LanCreateInvite"] = ("Create invitation", "生成邀请"),
        ["LanCancelInvite"] = ("Cancel invitation", "取消邀请"),
        ["LanCopy"] = ("Copy invitation", "复制邀请"),
        ["LanPair"] = ("Pair device", "配对设备"),
        ["LanPairHint"] = ("Discovery does not establish trust. Paste an invitation obtained directly from the other device; its certificate is verified before sending credentials.", "发现设备不代表信任。请粘贴直接从对方设备取得的邀请；发送凭据前会核验其证书。"),
        ["LanDiscover"] = ("Find nearby devices", "发现附近设备"),
        ["LanPasteInvite"] = ("Paste the private pairing invitation", "粘贴私密配对邀请"),
        ["LanSource"] = ("Lyrics source", "歌词来源"),
        ["LanUseSource"] = ("Show this device’s lyrics", "显示此设备的歌词"),
        ["LanLocalSource"] = ("Use this device’s player", "使用本机播放器"),
        ["LanForget"] = ("Forget remote device", "忘记远端设备"),
        ["LanAuthorized"] = ("Devices allowed to access this device", "获准访问本机的设备"),
        ["LanRevokeHint"] = ("Revoking a device immediately blocks its requests. Forgetting a remote source only removes the saved connection on this device.", "撤销授权会立即阻止该设备访问。忘记远端设备仅删除本机保存的连接。"),
        ["LanRevoke"] = ("Revoke access", "撤销授权"),
        ["LanRefresh"] = ("Refresh devices", "刷新设备"),
        ["LanControl"] = ("Playback and lyrics", "播放控制与歌词"),
        ["LanReadOnly"] = ("Lyrics only", "只读歌词"),
        ["LanEnableFirst"] = ("Save with sharing enabled before creating an invitation.", "请先启用共享并保存，再生成邀请。"),
        ["LanNoAddress"] = ("No private LAN IPv4 address was found. Connect to your local network first.", "未找到局域网 IPv4 地址，请先连接本地网络。"),
        ["LanFailed"] = ("Could not complete the connection. Check sharing, invitation expiry, certificate and firewall. No insecure fallback is used.", "连接未完成。请检查共享开关、邀请有效期、证书和防火墙；不会降级为不安全连接。"),
        ["LanWorking"] = ("Connecting…", "正在连接…"),
        ["LanNoneFound"] = ("No devices found. Enable sharing on the other device. An invitation can still connect directly if broadcast is blocked.", "未发现设备。请在对方设备开启共享；若广播被阻止，仍可用邀请直接连接。"),
        ["LanPaired"] = ("Paired. Select the device as your lyrics source when ready.", "配对成功，可选择该设备作为歌词来源。"),
        ["LanOffline"] = ("Selected device is offline or access was revoked", "所选设备离线或授权已被撤销"),
        ["LanSelfPair"] = ("This invitation belongs to this device. Local apps do not need pairing.", "这是本机的邀请，本机应用之间无需配对。"),
        ["LanPairFirst"] = ("Pair this device before selecting it.", "请先配对该设备。"),
        ["LanUsePairing"] = ("Remote devices must use HTTPS pairing: config lan. Plain HTTP and UDP control are local only.", "远端设备请使用 HTTPS 配对：config lan。明文 HTTP 和 UDP 控制仅限本机。"),
        ["LanCliUsage"] = ("config lan [on|off|name NAME|ports HTTPS UDP|discover|invite IP [--control]|pair [--stdin]|cancel|peers|source ID|source local|revoke ID|forget ID]", "config lan [on|off|name 名称|ports HTTPS端口 UDP端口|discover|invite IP [--control]|pair [--stdin]|cancel|peers|source ID|source local|revoke ID|forget ID]"),
        ["LanMenu"] = ("LAN devices: 1. Toggle sharing  2. Discover  3. Invite  4. Pair  5. Lyrics source  6. Revoke access  7. Cancel invitation  Enter. Back", "局域网设备：1. 切换共享  2. 发现设备  3. 邀请  4. 配对  5. 歌词来源  6. 撤销授权  7. 取消邀请  回车返回"),
        ["LanControlPrompt"] = ("Allow playback and favorites control? [y/N]", "允许控制播放与收藏？[y/N]"),
        ["LanSourcePrompt"] = ("Enter the paired device ID, or local for this device:", "输入已配对设备 ID，或 local 使用本机："),
        ["LanRevokePrompt"] = ("Enter the access ID to revoke:", "输入要撤销的授权 ID："),
        ...
        ["ChoosePreferences"] = ("Choose [1/2/3/4]: ", "请选择 [1/2/3/4]："),
